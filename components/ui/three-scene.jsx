...
let group, cubeLocations = [], activeLines = [], connectedVertices = new Map(), uniqueConnections = new Set();
...
let target = new THREE.WebGLRenderTarget(size.width, size.height, {
        samples: 8
      });

  const renderScene = new RenderPass(scene, camera);
  const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 2, 0.8, 0.65);
  ...
  function animate() {
    ...
    activeLines.forEach(lineObj => {
      ...
      if (!lineObj.line.userData.fadingOut) {
        if (elapsed < FADE_IN_DURATION) {
          lineObj.line.material.uniforms.opacity.value = easeInOutQuad(elapsed / FADE_IN_DURATION);
        } else {
          lineObj.line.material.uniforms.opacity.value = 1.0;
        }
      } else {
        const fadeOutElapsed = elapsedTime - lineObj.line.userData.fadeOutStartTime;
        if (fadeOutElapsed < FADE_OUT_DURATION) {
          lineObj.line.material.uniforms.opacity.value = 1.0 - easeInOutQuad(fadeOutElapsed / FADE_OUT_DURATION);
        } else {
    ...
    if (activeLines.length > 0 && activeLines.every(lineObj => lineObj.line.material.uniforms.opacity.value === 0.0)) {
      resetConnections();
    }
...
function createCubeWithDottedEdgesAndSmallCubes() {
  ...
  const vertexGeometry = new THREE.BoxGeometry(VERTEX_SIZE, VERTEX_SIZE, VERTEX_SIZE);
  const vertexMaterial = new THREE.MeshBasicMaterial({
    color: 0xffffff,
    emissive: 0xffffff, // Emissive color for bloom effect
  ...
  const instancedMesh = new THREE.InstancedMesh(vertexGeometry, vertexMaterial, uniqueVertices.length);
    ...
    onBeforeCompile: shader => {
      ...
      `.replace(
        ...
        `#include <color_vertex>
          ...
          d = clamp(d, 0., 1.);
          vColor = vec3(0.25, 0.25, 0.25); // Set the color to grey
        `
      ).replace(
        ...
        `#include <begin_vertex>
          ...
          transformed = clamp(transformed, vec3(-${CUBE_SIZE / 2}), vec3(${CUBE_SIZE / 2}));
        `
      );
...
function createGrid() {
  ...
  for (let i = 0; i < GRID_SIZE; i++) {
    for (let j = 0; j < GRID_SIZE; j++) {
      if ((i + j) % 2 === 1) {
        const cubeWithDottedEdgesAndSmallCubes = createCubeWithDottedEdgesAndSmallCubes();
        cubeWithDottedEdgesAndSmallCubes.position.set(0, i * SPACING - ((GRID_SIZE - 1) * SPACING) / 2, j * SPACING - ((GRID_SIZE - 1) * SPACING) / 2);
        group.add(cubeWithDottedEdgesAndSmallCubes);
        cubeLocations.push({ cube: cubeWithDottedEdgesAndSmallCubes, position: { x: 0, y: i, z: j }, gu: cubeWithDottedEdgesAndSmallCubes.userData.gu });
      }
    }
  }
...
function createLine(fromVertex, toVertex) {
  const lineGeometry = new THREE.BufferGeometry().setFromPoints([fromVertex, toVertex]);
  const line = new THREE.Line(lineGeometry, lineMaterial.clone());
  scene.add(line);

  // Find the cube that contains the `toVertex`
  const containingCube = cubeLocations.find(location => {
    return VERTEX_OFFSETS.some(offset => offset.position.clone().add(location.cube.position).equals(toVertex));
  });

  if (containingCube) {
    setTimeout(() => {
      transitionParticleColors(containingCube.cube.userData.particleCube, containingCube.gu);
    }, TRANSITION_DELAY);
  }
...
function initiateNewConnections(currentVertex, depth = 0) {
  ...
  newVertices.forEach((vertex, i) => {
    ...
    if (!connectedVertices.get(currentVertex).has(vertex) && !uniqueConnections.has(connectionKey)) {
      setTimeout(() => {
        const line = createLine(currentVertex, vertex);
        activeLines.push({ line, startTime: clock.getElapsedTime(), fadingOut: false, fadeOutStartTime: null });
      ...
      }, i * 65); // delay between connections
...
function resetConnections() {
  ...
  currentStartCubeIndex = (currentStartCubeIndex + 1) % cubeLocations.length;

  const startCube = cubeLocations[currentStartCubeIndex];
  const startVertex = VERTEX_OFFSETS[START_VERTEX_INDEX].position.clone().add(startCube.cube.position);
  setTimeout(() => {
    initiateNewConnections(startVertex);
  }, 600); // Delay between iterations
}
...
function transitionParticleColors(particleCube, gu) {
  const transitionColor = new THREE.Color(1, 1, 1); // Yellow color for transition
  // Nice colors are (1, 0, 0), (0.7, 0.9, 1)
  const originalColor = new THREE.Color(0.4, 0.4, 0.4); // Original color

  let start = { r: gu.color.value.r, g: gu.color.value.g, b: gu.color.value.b };
  let end = { r: transitionColor.r, g: transitionColor.g, b: transitionColor.b };
  let duration = FADE_IN_DURATION * 1000 * 4; // Particle color transition duration
  let elapsed = 0;
  ...
  function animateColorTransition() {
    ...
    if (elapsed < duration) {
      ...
      setTimeout(() => {
        start = { r: gu.color.value.r, g: gu.color.value.g, b: gu.color.value.b };
        end = { r: originalColor.r, g: originalColor.g, b: originalColor.b };
        elapsed = 0;
